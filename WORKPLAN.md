...
- 2026-05-10: **좋아요/저장 DB 연동 + 저장한 후기 페이지 + 커뮤니티 UX 다듬기 라운드**. 큰 묶음으로:
  ...
  - **BETA 배지 (작업 중)**: AppShell에 `showBeta` prop 신설 + 헤더 브랜드 우측에 주황 pill `BETA` 배지(`.brand-beta`). 다크 테마용 톤 보정(`dark-core-home.css`)까지. 출시 초기 사용자에게 *"미완성 부분이 있을 수 있음"* 신호를 주려는 의도. 현재 홈 화면에만 적용, 나머지 화면 적용 여부는 미확정 → 미커밋 상태.

진행 상태: 출시 직전 폴리시 라운드 완료 + 인터랙션 깊이 보강 라운드 완료. BETA 배지는 적용 범위 결정 후 일괄 커밋 예정. 남은 항목 — Phase 9 잔여 QA(접근성/lint/build), Phase 10 배포 점검, Phase 11 인수 문서.
